...
export class AuthService {
  ...
  async adminLogout(adminIdx: number): Promise<void> {
    const adminCnt: number = await this.userRepository.getAdminCountByIdx(adminIdx);
    if (adminCnt !== 1) {
      throw new BadRequestException('올바른 어드민이 아닙니다.');
    }

    await this.authRepository.deleteAdminToken(adminIdx);
