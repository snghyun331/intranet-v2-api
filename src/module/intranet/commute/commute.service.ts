...
export class CommuteService {
  ...
  async checkInWork(userIdx: number, checkInDto: CheckInDto, manager: EntityManager): Promise<void> {
    // 오늘의 출근 정보가 있는지 확인
    ...
    await this.commuteRepository.checkInWork(userIdx, checkInDto, manager);

    return;
  }

  async checkOutWork(userIdx: number, checkOutDto: CheckOutDto, manager: EntityManager): Promise<void> {
    // 오늘의 출근 정보가 있는지 확인
    ...
    await this.commuteRepository.checkOutWork(userIdx, checkOutDto, manager);
